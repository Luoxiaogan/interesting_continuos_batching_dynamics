...
def plot_state_differences(sim, data_series, jump=1, start_step=10):
    ...
    # 定义三种类型的颜色方案
    color_maps = [plt.cm.Blues, plt.cm.Reds, plt.cm.Greens]
    ...
    colors_type0 = color_maps[0](np.linspace(0.4, 0.9, len(sim.type_valid_lengths[0])))
    ...
    if sim.num_types > 1:
        colors_type1 = color_maps[1](np.linspace(0.3, 0.9, len(sim.type_valid_lengths[1])))
...
def plot_admission_eviction(sim, start_step=10):
    ...
    """
    # 创建图表 - 调整为支持3种类型
    num_rows = min(sim.num_types, 3)  # 最多显示3个类型
    fig_ae = plt.figure(figsize=(16, 5 * num_rows))  # 动态调整高度
    gs = GridSpec(num_rows, 1, figure=fig_ae, hspace=0.3)
    ...
    # 为每个类型创建子图
    for type_idx in range(min(sim.num_types, 3)):  # 最多显示3个类型
...
def plot_state_evolution(sim, num_steps, start_step=10, compute_differences=True, jump=1):
    ...
    type_colors = []
    for type_idx in range(sim.num_types):
        if type_idx < len(color_maps):
            n_lengths = len(sim.type_valid_lengths[type_idx])
            colors = color_maps[type_idx](np.linspace(0.3, 0.9, n_lengths))
            type_colors.append(colors)
    ...
    markers = ['o', 's', '^', 'D', 'v', '<', '>', 'p']
    for type_idx in range(sim.num_types):
        for i, length in enumerate(sorted(sim.type_valid_lengths[type_idx])):
            key = f"Type{type_idx}_L{length}"
            if key in data_series:
                ax1.plot(batches, data_series[key]['values'],
                        label=f"Type {type_idx}, Length {length}",
                        color=type_colors[type_idx][i] if type_idx < len(type_colors) else 'gray',
                        linewidth=2, marker=markers[type_idx % len(markers)], markersize=3)
    ...
    # 为前两个类型创建单独的子图
    axes = [ax2, ax3]
    for plot_idx in range(min(2, sim.num_types)):
        ax = axes[plot_idx]
        l0, l1 = sim.request_types[plot_idx]
        ax.set_title(f'Type {plot_idx} (l0={l0}, l1={l1})', fontweight='bold')

        for i, length in enumerate(sorted(sim.type_valid_lengths[plot_idx])):
            key = f"Type{plot_idx}_L{length}"
            if key in data_series:
                ax.plot(batches, data_series[key]['values'],
                       label=f"Length {length}",
                       color=type_colors[plot_idx][i] if plot_idx < len(type_colors) else 'gray',
                       linewidth=2, marker=markers[plot_idx % len(markers)])
    ...
    # 如果有第三个类型，创建额外的图
    if sim.num_types > 2:
        ax4_extra = fig.add_subplot(gs[2, 0])
        l0, l1 = sim.request_types[2]
        ax4_extra.set_title(f'Type 2 (l0={l0}, l1={l1})', fontweight='bold')

        for i, length in enumerate(sorted(sim.type_valid_lengths[2])):
            key = f"Type2_L{length}"
            if key in data_series:
                ax4_extra.plot(batches, data_series[key]['values'],
                             label=f"Length {length}",
                             color=type_colors[2][i] if 2 < len(type_colors) else 'gray',
                             linewidth=2, marker=markers[2 % len(markers)])

        ax4_extra.set_xlabel('Batch Number')
        ax4_extra.set_ylabel('Number of Requests')
        ax4_extra.legend()
        ax4_extra.grid(True, alpha=0.3)
...
def analyze_steady_state(sim, data_series, last_n_batches=20):
    ...
    type_totals = []
    for type_idx in range(sim.num_types):
        type_total = sum(np.mean(data_series[f"Type{type_idx}_L{l}"]['values'][-last_n_batches:])
                       for l in sim.type_valid_lengths[type_idx])
        type_totals.append(type_total)

    total_all = sum(type_totals)

    print(f"\nType distribution in steady state:")
    for type_idx in range(sim.num_types):
        if total_all > 0:
            percentage = type_totals[type_idx] / total_all * 100
        else:
            percentage = 0
...
if __name__ == "__main__":
    ...
    a = 400
    request_types = [
        (a, 3),  # Type 0: 初始长度400，decode 3步
        (a,6),  # Type 1: 初始长度400，decode 5步
        (a, 9),  # Type 2: 初始长度400，decode 7步
    ]

    # 初始状态 - 为三种类型设置初始分布
    X0 = {
        a:     [1.0, 1.0, 1.0],    # 所有类型在初始长度
        a+1:   [1.0, 1.0, 1.0],    # decode步骤1
        a+2:   [1.0, 1.0, 1.0],    # decode步骤2 (Type 0即将完成)
        a+3:   [0.0, 1.0, 1.0],    # decode步骤3 (Type 0已完成)
        a+4:   [0.0, 1.0, 1.0],    # decode步骤4 (Type 1即将完成)
        a+5:   [0.0, 0.0, 1.0],    # decode步骤5 (Type 1已完成)
        a+6:   [0.0, 0.0, 1.0],    # decode步骤6 (Type 2即将完成)
        a+7:   [0.0, 0.0, 1.0],    # decode步骤6 (Type 2即将完成)
        a+8:   [0.0, 0.0, 1.0],    # decode步骤6 (Type 2即将完成)
    }
    ...
    sim = VisualizedScheduler(
        ...
        B= 5000,  # GPU容量
        X0=X0,
        arrival_rates=[8.0, 6.0, 4.0],  # 三种类型的到达率
        b0=0.1,
        b1=0.001  # 减小b1以适应更大的长度
    )
    
    # 运行并可视化
    num_steps = 2000  # 减少步数以便快速测试
    jump = 1  # 可以调整jump参数来看不同步长的差异
    fig, data_series, fig_diff = plot_state_evolution(sim, num_steps, start_step=num_steps-50,
